...
from plotter.rpc.full_node_rpc_client import FullNodeRpcClient
...
from plotter.util.misc import format_bytes
from plotter.util.misc import format_minutes
...
async def summary(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    if amounts is not None:
        print(f"Total plotter farmed: {amounts['farmed_amount'] / units['plotter']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['plotter']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['plotter']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'plotter start wallet' and 'plotter wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'plotter wallet show'")
    else:
        print("Note: log into your key using 'plotter wallet show' to see rewards for each key")
