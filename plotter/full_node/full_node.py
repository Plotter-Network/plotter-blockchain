...
import plotter.server.ws_connection as ws  # lgtm [py/import-and-import-from]
from plotter.consensus.block_creation import unfinished_block_to_full_block
from plotter.consensus.block_record import BlockRecord
from plotter.consensus.blockchain import Blockchain, ReceiveBlockResult
from plotter.consensus.constants import ConsensusConstants
from plotter.consensus.difficulty_adjustment import get_next_sub_slot_iters_and_difficulty
...
from plotter.consensus.pot_iterations import calculate_sp_iters
from plotter.full_node.block_store import BlockStore
from plotter.full_node.bundle_tools import detect_potential_template_generator
from plotter.full_node.coin_store import CoinStore
from plotter.full_node.full_node_store import FullNodeStore
from plotter.full_node.mempool_manager import MempoolManager
from plotter.full_node.signage_point import SignagePoint
from plotter.full_node.sync_store import SyncStore
from plotter.full_node.weight_proof import WeightProofHandler
from plotter.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from plotter.protocols.full_node_protocol import (
    RejectBlocks,
    RequestBlocks,
    RespondBlock,
    RespondBlocks,
...
from plotter.protocols.protocol_message_types import ProtocolMessageTypes
from plotter.server.node_discovery import FullNodePeers
from plotter.server.outbound_message import Message, NodeType, make_msg
from plotter.server.server import PlotterServer
from plotter.types.blockchain_format.classgroup import ClassgroupElement
from plotter.types.blockchain_format.pool_target import PoolTarget
...
from plotter.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
from plotter.types.end_of_slot_bundle import EndOfSubSlotBundle
from plotter.types.full_block import FullBlock
from plotter.types.header_block import HeaderBlock
from plotter.types.mempool_inclusion_status import MempoolInclusionStatus
from plotter.types.spend_bundle import SpendBundle
from plotter.types.unfinished_block import UnfinishedBlock
from plotter.util.bech32m import encode_puzzle_hash
from plotter.util.db_wrapper import DBWrapper
from plotter.util.errors import ConsensusError, Err
from plotter.util.ints import uint8, uint32, uint64, uint128
from plotter.util.path import mkdir, path_from_root
from plotter.util.safe_cancel_task import cancel_task_safe
from plotter.util.profiler import profile_task
...
class FullNode:
    ...
    def set_server(self, server: PlotterServer):
        self.server = server
        dns_servers = []
        try:
        ...
        elif self.config["port"] == 4242:
            # If `dns_servers` misses from the `config`, hardcode it if we're running mainnet.
            dns_servers.append("dns-introducer.plotter.network")
        try:
    ...
    async def short_sync_batch(self, peer: ws.WSPlotterConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: ws.WSPlotterConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ):
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSPlotterConnection):
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[ws.WSPlotterConnection] = None
    ):
        """
    ...
    async def on_connect(self, connection: ws.WSPlotterConnection):
        """
    ...
    def on_disconnect(self, connection: ws.WSPlotterConnection):
    ...
    async def peak_post_processing(
        self, block: FullBlock, record: BlockRecord, fork_height: uint32, peer: Optional[ws.WSPlotterConnection]
    ):
        """
    ...
    async def respond_block(
        ...
        peer: Optional[ws.WSPlotterConnection] = None,
    ) -> Optional[Message]:
        """
    ...
    async def respond_unfinished_block(
        ...
        peer: Optional[ws.WSPlotterConnection],
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'pltr')}, "
                f"validation time: {validation_time}, "
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[ws.WSPlotterConnection] = None
    ) -> Optional[Message]:
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: ws.WSPlotterConnection
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: ws.WSPlotterConnection):
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: ws.WSPlotterConnection):
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
    ...
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: ws.WSPlotterConnection):
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
