...
from plotter.rpc.rpc_client import RpcClient
...
class WalletRpcClient(RpcClient):
    """
    Client to Plotter RPC, connects to a local wallet. Uses HTTP/JSON, and converts back from
    JSON into native python objects before returning. All api calls use POST requests.
    Note that this is not the same as the peer protocol, or wallet protocol (which run Plotter's
