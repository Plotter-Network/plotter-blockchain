...
class ConsensusConstants:
    ...
    # Forks of plotter should change this value to provide replay attack protection
    AGG_SIG_ME_ADDITIONAL_DATA: bytes
    GENESIS_PRE_FARM_POOL_PUZZLE_HASH: bytes32  # Remained from Chia premine. As genesis block will not provide reward 
    # it does nothing, but the hashed value is sha256("Plotter")
    GENESIS_PRE_FARM_FARMER_PUZZLE_HASH: bytes32  # Remained from Chia premine. As genesis block will not provide 
    # reward it does nothing, but the hashed value is sha256("Plotter goes BRR")
    ...
    # Max coin amount uint(1 << 64). This allows coin amounts to fit in 64 bits. This is around 18M plotter.
    MAX_COIN_AMOUNT: int
