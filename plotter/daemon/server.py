...
from plotter.cmds.init_funcs import plotter_init
from plotter.daemon.windows_signal import kill
from plotter.server.server import ssl_context_for_root, ssl_context_for_server
from plotter.ssl.create_ssl import get_mozilla_ca_crt
from plotter.util.plotter_logging import initialize_logging
...
from plotter.util.service_groups import validate_service
from plotter.util.setproctitle import setproctitle
from plotter.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Plotter.")
    quit()
...
service_plotter = "plotter plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "plotter": "plotter",
        "plotter_wallet": "start_wallet",
        "plotter_full_node": "start_full_node",
        "plotter_harvester": "start_harvester",
        "plotter_farmer": "start_farmer",
        "plotter_introducer": "start_introducer",
        "plotter_timelord": "start_timelord",
        "plotter_timelord_launcher": "timelord_launcher",
        "plotter_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("plotter daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered PLOTTER_ROOT
    os.environ["PLOTTER_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with PLOTTER_ROOT: {os.environ['PLOTTER_ROOT']}")
...
async def async_run_daemon(root_path: Path) -> int:
    plotter_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("plotter_daemon")
