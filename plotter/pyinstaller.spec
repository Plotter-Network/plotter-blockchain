...
ROOT = pathlib.Path(importlib.import_module("plotter").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `plotter` file name (which is the executable)
    and the `plotter` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `plotter` directory, since the names collide.
    ...
    for data in analysis.datas:
        if str(data[0]).startswith("plotter/"):
            zipped.append(data)
        else:
            datas.append(data)
...
version_data = copy_metadata(get_distribution("plotter-blockchain"))[0]
...
# TODO: collapse all these entry points into one `plotter_exec` entrypoint that accepts the server as a parameter

entry_points = ["plotter.cmds.plotter"] + [f"plotter.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "plotter.util.bip39"])
...
datas.append((f"{ROOT}/plotter/util/english.txt", "plotter/util"))
datas.append((f"{ROOT}/plotter/util/initial-config.yaml", "plotter/util"))
datas.append((f"{ROOT}/plotter/wallet/puzzles/*.hex", "plotter/wallet/puzzles"))
datas.append((f"{ROOT}/plotter/ssl/*", "plotter/ssl"))
...
add_binary("plotter", f"{ROOT}/plotter/cmds/plotter.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/plotter/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/plotter/server/start_{server}.py", COLLECT_ARGS)
