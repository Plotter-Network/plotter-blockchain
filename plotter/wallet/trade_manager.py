...
from plotter.types.coin_solution import CoinSolution
...
from plotter.wallet.cc_wallet import cc_utils
from plotter.wallet.cc_wallet.cc_utils import CC_MOD, SpendableCC, spend_bundle_for_spendable_ccs, uncurry_cc
from plotter.wallet.cc_wallet.cc_wallet import CCWallet
from plotter.wallet.puzzles.genesis_by_coin_id_with_0 import genesis_coin_id_for_genesis_coin_checker
from plotter.wallet.trade_record import TradeRecord
from plotter.wallet.trading.trade_status import TradeStatus
from plotter.wallet.trading.trade_store import TradeStore
from plotter.wallet.transaction_record import TransactionRecord
from plotter.wallet.util.trade_utils import (
...
# from plotter.wallet.cc_wallet.debug_spend_bundle import debug_spend_bundle
            ...
            for id in offer.keys():
                ...
                if isinstance(wallet, CCWallet):
                    ...
                    if spend_bundle is None:
                        ...
                        to_exclude = spend_bundle.removals()
                    new_spend_bundle = await wallet.create_spend_bundle_relative_plotter(amount, to_exclude)
                else:
    ...
    async def respond_to_offer(self, file_path: Path) -> Tuple[bool, Optional[TradeRecord], Optional[str]]:
        ...
        for coinsol in offer_spend_bundle.coin_solutions:
            ...
            if r:
                ...
                # standard plotter coin
                unspent = await self.wallet_state_manager.get_spendable_coins_for_wallet(1)
                if coinsol.coin in [record.coin for record in unspent]:
                    return False, None, "can't respond to own offer"
                if plotter_discrepancy is None:
                    plotter_discrepancy = get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                else:
                    plotter_discrepancy += get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                coinsols.append(coinsol)

        plotter_spend_bundle: Optional[SpendBundle] = None
        if plotter_discrepancy is not None:
            plotter_spend_bundle = await self.wallet_state_manager.main_wallet.create_spend_bundle_relative_plotter(
                plotter_discrepancy, []
            )
            if plotter_spend_bundle is not None:
                for coinsol in coinsols:
                    plotter_spend_bundle.coin_solutions.append(coinsol)
        ...
        for colour in cc_discrepancies.keys():
            if cc_discrepancies[colour] < 0:
                ...
                if plotter_spend_bundle is None:
                    to_exclude: List = []
                else:
                    to_exclude = plotter_spend_bundle.removals()
                ...
                if my_cc_spends is None or my_cc_spends == set():
                    ...
                    if zero_spend_bundle is None:
                        return (
                            False,
                            None,
                            "Unable to generate zero value coin. Confirm that you have plotter available",
                        )
        ...
        now = uint64(int(time.time()))
        if plotter_spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, plotter_spend_bundle])
            # debug_spend_bundle(spend_bundle)
            if plotter_discrepancy < 0:
                tx_record = TransactionRecord(
                    confirmed_at_height=uint32(0),
                    created_at_time=now,
                    to_puzzle_hash=token_bytes(),
                    amount=uint64(abs(plotter_discrepancy)),
                    fee_amount=uint64(0),
                    confirmed=False,
                    sent=uint32(10),
                    spend_bundle=plotter_spend_bundle,
                    additions=plotter_spend_bundle.additions(),
                    removals=plotter_spend_bundle.removals(),
                    wallet_id=uint32(1),
                    sent_to=[],
                    trade_id=std_hash(spend_bundle.name() + bytes(now)),
                    type=uint32(TransactionType.OUTGOING_TRADE.value),
                    name=plotter_spend_bundle.name(),
                )
            else:
                tx_record = TransactionRecord(
                    confirmed_at_height=uint32(0),
