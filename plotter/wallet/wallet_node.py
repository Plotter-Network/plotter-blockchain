...
from plotter.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from plotter.protocols.protocol_message_types import ProtocolMessageTypes
from plotter.protocols.wallet_protocol import (
...
from plotter.util.errors import Err, ValidationError
from plotter.util.ints import uint32, uint128
from plotter.util.keychain import Keychain
from plotter.util.lru_cache import LRUCache
from plotter.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from plotter.util.path import mkdir, path_from_root
from plotter.wallet.block_record import HeaderBlockRecord
from plotter.wallet.derivation_record import DerivationRecord
from plotter.wallet.settings.settings_objects import BackupInitialized
from plotter.wallet.transaction_record import TransactionRecord
from plotter.wallet.util.backup_utils import open_backup_file
from plotter.wallet.util.wallet_types import WalletType
from plotter.wallet.wallet_action import WalletAction
from plotter.wallet.wallet_blockchain import ReceiveBlockResult
from plotter.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'plotter keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSPlotterConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSPlotterConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSPlotterConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSPlotterConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(self, peer: WSPlotterConnection, block_i, additions) -> Optional[List[Coin]]:
        if len(additions) > 0:
    ...
    async def get_removals(self, peer: WSPlotterConnection, block_i, additions, removals) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
