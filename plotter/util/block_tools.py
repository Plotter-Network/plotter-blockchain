...
from plotter.cmds.init_funcs import create_all_ssl, create_default_plotter_config
from plotter.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from plotter.plotting.create_plots import create_plots
from plotter.consensus.block_creation import create_unfinished_block, unfinished_block_to_full_block
...
from plotter.consensus.default_constants import DEFAULT_CONSTANTS
from plotter.consensus.deficit import calculate_deficit
from plotter.consensus.full_block_to_block_record import block_to_block_record
from plotter.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from plotter.consensus.pot_iterations import (
    calculate_ip_iters,
...
from plotter.consensus.vdf_info_computation import get_signage_point_vdf_info
from plotter.full_node.signage_point import SignagePoint
from plotter.plotting.plot_tools import PlotInfo, load_plots, parse_plot_info
...
from plotter.types.blockchain_format.proof_of_space import ProofOfSpace
from plotter.types.blockchain_format.sized_bytes import bytes32
from plotter.types.blockchain_format.slots import (
...
from plotter.types.generator_types import BlockGenerator, CompressorArg
...
from plotter.util.block_cache import BlockCache
from plotter.util.config import load_config, save_config
from plotter.util.hash import std_hash
from plotter.util.ints import uint8, uint32, uint64, uint128
from plotter.util.keychain import Keychain, bytes_to_mnemonic
from plotter.util.path import mkdir
from plotter.util.vdf_prover import get_vdf_info_and_proof
from plotter.util.wallet_tools import WalletTool
from plotter.wallet.derive_keys import (
...
class BlockTools:
    ...
    def __init__(
        ...
        if len(self.pool_pubkeys) == 0 or len(farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `plotter generate keys`")
    ...
    def init_plots(self, root_path: Path):
        ...
        try:
            ...
            args.pool_contract_address = encode_puzzle_hash(self.pool_ph, "pltr")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("PLOTTER_ROOT", "~/.plotter/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
