...
from plotterpos import DiskPlotter

from plotter.consensus.coinbase import create_puzzlehash_for_pk
from plotter.plotting.plot_tools import stream_plot_info_ph, stream_plot_info_pk
from plotter.protocols import farmer_protocol
from plotter.rpc.farmer_rpc_api import FarmerRpcApi
from plotter.rpc.farmer_rpc_client import FarmerRpcClient
from plotter.rpc.harvester_rpc_api import HarvesterRpcApi
from plotter.rpc.harvester_rpc_client import HarvesterRpcClient
from plotter.rpc.rpc_server import start_rpc_server
...
from plotter.util.block_tools import get_plot_dir
...
from plotter.wallet.derive_keys import master_sk_to_wallet_sk
...
class TestRpc:
    ...
    async def test1(self, simulation):
        ...
        try:
            ...
            await client.set_reward_targets(encode_puzzle_hash(new_ph, "pltr"), encode_puzzle_hash(new_ph_2, "pltr"))
            ...
            await client.set_reward_targets(None, encode_puzzle_hash(new_ph_3, "pltr"))
            ...
            assert config["farmer"]["pltr_target_address"] == encode_puzzle_hash(new_ph, "pltr")
            assert config["pool"]["pltr_target_address"] == encode_puzzle_hash(new_ph_3, "pltr")

            new_ph_3_encoded = encode_puzzle_hash(new_ph_3, "pltr")
