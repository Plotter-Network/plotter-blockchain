...
from plotter.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from plotter.full_node.full_node_api import FullNodeAPI
from plotter.server.start_farmer import service_kwargs_for_farmer
from plotter.server.start_full_node import service_kwargs_for_full_node
from plotter.server.start_harvester import service_kwargs_for_harvester
from plotter.server.start_introducer import service_kwargs_for_introducer
from plotter.server.start_service import Service
from plotter.server.start_timelord import service_kwargs_for_timelord
from plotter.server.start_wallet import service_kwargs_for_wallet
from plotter.simulator.start_simulator import service_kwargs_for_full_node_simulator
from plotter.timelord.timelord_launcher import kill_processes, spawn_process
from plotter.types.peer_info import PeerInfo
from plotter.util.bech32m import encode_puzzle_hash
from plotter.util.block_tools import BlockTools, test_constants
from plotter.util.hash import std_hash
from plotter.util.ints import uint16, uint32
...
async def setup_farmer(
    ...
    config["pltr_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "pltr")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["pltr_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "pltr")
